...
public class UnitTestR
{
    ...
    public UnitTestR(ITestOutputHelper testOutputHelper)
    {
        ...
        _testOutputHelper.WriteLine("FileName = {0}", Path.GetFileName(FileName));
        _testOutputHelper.WriteLine("Directory = {0}", Path.GetDirectoryName(FileName));
        _testOutputHelper.WriteLine("FileModifyDate = {0}", File.GetLastWriteTime(FileName));
        _testOutputHelper.WriteLine("FileAccessDate = {0}", File.GetLastAccessTime(FileName));
        _testOutputHelper.WriteLine("FileCreateDate = {0}", File.GetCreationTime(FileName));
    }
    ...
    public void DirectDumpTest()
    {
        ...
        _testOutputHelper.WriteLine("FileSize {0}", fileStream.Length);
    ...
    public void ParseCr3Test()
    {
        ...
        var msg = Parse(binaryReader, 0L, length, " ");
        _testOutputHelper.WriteLine(msg);

        Assert.Equal(binaryReader.BaseStream.Length, binaryReader.BaseStream.Position);
    }

    private static string Parse(BigEndianBinaryReader binaryReader, long start, long size, string depth)
    {
        ...
        Assert.Equal(binaryReader.BaseStream.Position, start);
        ...
        while (binaryReader.BaseStream.Position < start + size)
        {
            ...
            switch (chunkName)
            {
                ...
                case "ftyp": // File Type Box
                    ...
                    builder.AppendFormat("{0}ftyp: {1} {2}: ", depth, majorBrand, version);
                    foreach (var tag in tags) builder.AppendFormat("{0}, ", tag);
                    builder.AppendLine();
                    break;

                case "moov": // Container box whose sub-boxes define the metadata for presentation
                    builder.AppendFormat("{0}moov:", depth);
                    builder.AppendLine();
                    var pp = Parse(binaryReader, s1 + b0, length - b0, depth + " ");
                    builder.Append(pp);
                    break;
                ...
                case "CNCV": // Canon Compressor Version
                    ...
                    builder.AppendFormat("{0}CNCV: {1}", depth, x11);
                    builder.AppendLine();
                    // if (CRM) 
                ...
                case "CCTP": // Canon CR3 Track Pointers
                    ...
                    builder.AppendFormat("{0}CCTP: {1}, {2}, {3}", depth, cctp0, cctp1, cctp2);
                    builder.AppendLine();
                    var p2 = Parse(binaryReader, s1 + b0 + 12, length - b0 - 12, depth + " ");
                    builder.Append(p2);
                    break;
                ...
                case "CCDT": // Canon Compressor Data Type
                    ...
                    builder.AppendFormat("{0}CCDT type {1}, {2}, {3}", depth, ccdt0, ccdt1, ccdt2);
                    builder.AppendLine();
                    break;

                case "CTBO": // Canon Trak B Offsets
                    var l9 = binaryReader.ReadInt32();
                    builder.AppendFormat("{0}CTBO: {1}", depth, l9);
                    builder.AppendLine();
                    for (var i9 = 0; i9 < l9; i9++)
                    {
                        ...
                        builder.AppendFormat("{0}  {1}, 0x{2:X8}, 0x{3:X8}", depth, idx9, offset9, size9);
                        builder.AppendLine();
                    }
                ...
                case "CMT3":
                case "CMT4":
                    builder.AppendFormat("{0}{1}, {2} bytes", depth, chunkName, length - b0);
                    builder.AppendLine();
                ...
                case "THMB": // Thumbnail
                    ...
                    builder.AppendFormat("{0}THMB: {1}, {2}, 0x{3:X}, a={4}, m={5}, n={6}, {7} byte pad", depth, w8,
                        h8, s8, a8, m8, n8, pad8);
                    builder.AppendLine();
                    //foreach (var bp in p8) Console.Write("{0:X} ", bp);
                    //Console.WriteLine();
                    Assert.Equal(length, b0 + 16L + s8 + pad8);
                    Assert.Equal(binaryReader.BaseStream.Position, s1 + b0 + 16L + s8 + pad8);
                    break;
                ...
                case "trak": // Track: Embedded JPEG
                    ...
                    builder.AppendFormat("{0}{1}: {2}", depth, trakName, trakType);
                    builder.AppendLine();
                    break;
                ...
                case "mdia": // Media
                    builder.AppendFormat("{0}{1}, size 0x{2:X08}", depth, chunkName, length);
                    builder.AppendLine();
                    var p3 = Parse(binaryReader, s1 + b0, length - b0, depth + " ");
                    builder.Append(p3);
                    break;
                ...
                case "minf": // Media Information Container
                    builder.AppendFormat("{0}{1}, size 0x{2:X08}", depth, chunkName, length);
                    builder.AppendLine();
                    var p5 = Parse(binaryReader, s1 + b0, length - b0, depth + " ");
                    builder.Append(p5);
                    break;
                ...
                case "dinf": // Data Information Box
                    builder.AppendFormat("{0}{1}, size 0x{2:X08}", depth, chunkName, length);
                    builder.AppendLine();
                    var p6 = Parse(binaryReader, s1 + b0, length - b0, depth + " ");
                    builder.Append(p6);
                    break;
                ...
                case "dref": // Data Reference Box
                    ...
                    var p7 = Parse(binaryReader, s1 + b0 + 8, length - b0 - 8, depth + " ");
                    builder.Append(p7);
                    break;
                ...
                case "stbl": // Sample Table Box
                    builder.AppendFormat("{0}{1}, size 0x{2:X08}", depth, chunkName, length);
                    builder.AppendLine();
                    var p9 = Parse(binaryReader, s1 + b0, length - b0, depth + " ");
                    builder.Append(p9);
                    break;
                ...
                case "stsd": // Samble Descriptions, Codex Types, Init, etc.
                    ...
                    var p10 = Parse(binaryReader, s1 + b0 + 8, length - b0 - 8, depth + " ");
                    builder.AppendLine(p10);
                    break;
                ...
                case "CRAW": // Lossy Compression
                    ...
                    var p11 = Parse(binaryReader, s1 + b0 + 0x52, length - b0 - 0x52, depth + " ");
                    builder.Append(p11);
                    break;

                case "JPEG":
                    var a5 = binaryReader.ReadInt32();
                    builder.AppendFormat("{0}JPEG: {1}", depth, a5);
                    builder.AppendLine();
                    break;
                ...
                case "stsz": // Sample Size, Framing, size of jpeg picture #1 in mdat
                    ...
                    builder.AppendFormat("{0}stsz: {1}, {2}, {3}", depth, v2, s2, c2);
                    builder.AppendLine();
                    break;
                ...
                case "co64": // Pointer to Picture #1 inside mdat
                    ...
                    builder.AppendFormat("{0}co64: {1}, {2}, {3}, {4}", depth, v3, s3, c3, d3);
                    builder.AppendLine();
                    break;
                ...
                case "PRVW": // Preview
                    ...
                    builder.AppendFormat("{0}PRVW: {1}, {2}, 0x{3:X}, {4}, {5}", depth, w7, h7, s7, m7, n7);
                    builder.AppendLine();
                    Assert.Equal(length, b0 + 16 + s7);
                    Assert.Equal(binaryReader.BaseStream.Position, s1 + b0 + 16L + s7);
                    break;

                case "CDI1":
                    builder.AppendFormat("{0}CDI1: {1} bytes", depth, length - b0);
                    builder.AppendLine();
                    var pad10 = length - b0;
                    var p20 = binaryReader.ReadBytes((int) pad10);
                    break;

                case "url ":
                    var a4 = binaryReader.ReadInt32();
                    builder.AppendFormat("{0}url : {1}", depth, a4);
                    builder.AppendLine();
                    break;

                case "nmhd":
                    var a6 = binaryReader.ReadInt32();
                    builder.AppendFormat("{0}nmhd: {1}", depth, a6);
                    builder.AppendLine();
                    break;
                ...
                case "uuid":
                    ...
                    builder.AppendFormat("{0}uuid: {1}", depth, uuid);
                    builder.AppendLine();
                    switch (uuid.ToString())
                    {
                        case "87b6c085-0f82-e011-8111-f4ce462b6a48":
                            var p12 = Parse(binaryReader, s1 + b0 + 16, length - b0 - 16, depth + " ");
                            builder.Append(p12);
                            break;
                        case "cbcf7abe-a997-e842-9c71-999491e3afac": // xpacket data
                            var x4c = binaryReader.ReadBytes((int) (length - b0 - 16));
                            var x4text = Encoding.ASCII.GetString(x4c);
                            Assert.StartsWith("<?xpacket begin=", x4text);
                            Assert.EndsWith("<?xpacket end='w'?>", x4text);
                            // Console.WriteLine(x4text);
                            break;
                        case "5e2bf4ea-981c-884b-b9fb-b7dc406e4d16": // preview data, jpeg 1620x1080
                            var x3 = binaryReader.ReadInt64();
                            var p13 = Parse(binaryReader, s1 + b0 + 24, length - b0 - 24, depth + " ");
                            builder.Append(p13);
                            break;
                        default:
                            builder.AppendFormat("{0}** Unknown UUID {1}, {2} bytes", depth, uuid, length - b0 - 16);
                            builder.AppendLine();
                ...
                case "CTI1":
                    builder.AppendFormat("{0}CN: {1}, size 0x{2:X08}", depth, chunkName, length);
                    builder.AppendLine();
                    var data5 = binaryReader.ReadBytes(4);
                    // Assert.Equal(s1 + b0 + 4, binaryReader.BaseStream.Position);
                    var p14 = Parse(binaryReader, s1 + b0 + 4, length - b0 - 4, depth + " ");
                    builder.Append(p14);
                    break;
                ...
                default:
                    builder.AppendFormat("{0}** Unknown Chunk {1}, {2} bytes", depth, chunkName, length - b0);
                    builder.AppendLine();
            ...
            Assert.True(binaryReader.BaseStream.Position <= start + size);
        }

        builder.AppendFormat("{0}----", depth);
        builder.AppendLine();
