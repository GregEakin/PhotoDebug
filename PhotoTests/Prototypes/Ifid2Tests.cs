...
public class Ifid2Tests
{
    ...
    private static void DumpImage2(string fileName)
    {
        ...
        Assert.Equal(new ushort[] { 16, 16, 16 }, bitsPerSample);

        var compression = image.Entries.Single(e => e.TagId == 0x0103 && e.TagType == ImageFileEntry.TagTypes.UShort).ValuePointer;
        Assert.Equal(1u, compression);

        var photometricInterpretation = image.Entries.Single(e => e.TagId == 0x0106 && e.TagType == ImageFileEntry.TagTypes.UShort).ValuePointer;
        Assert.Equal(2u, photometricInterpretation);
        ...
        Assert.Equal(1u, planarConfiguration);
        ...
        Assert.Equal(new[] { 577u, 386u, 14u, 9u }, stuff);
    }
}
