    ...
    public class Canon5D4
    {
        ...
        private static void DumpImage3Raw(string fileName)
        {
            ...
            using (var binaryReader = new BinaryReader(fileStream))
            {
                ...
                CollectionAssert.AreEqual(new[] { (ushort)1, (ushort)3440, (ushort)3440 }, slices);

                binaryReader.BaseStream.Seek(offset, SeekOrigin.Begin);
                var startOfImage = new StartOfImage(binaryReader, offset, count);

                var startOfFrame = startOfImage.StartOfFrame;
                Assert.AreEqual(2272u, startOfFrame.ScanLines); // = 3840 + 110
                Assert.AreEqual(3440u, startOfFrame.SamplesPerLine); // = 5920 / 2
                Assert.AreEqual(13760, startOfFrame.Width); // = 5760 + 160

                Assert.AreEqual(14, startOfFrame.Precision); // RGGB

                // chrominance subsampling factors
                Assert.AreEqual(4, startOfFrame.Components.Length); // RGGB
                ...
                Assert.AreEqual(4, startOfFrame.Components[3].ComponentId);
                Assert.AreEqual(1, startOfFrame.Components[3].HFactor);
                Assert.AreEqual(1, startOfFrame.Components[3].VFactor);
                Assert.AreEqual(0, startOfFrame.Components[3].TableId);
                ...
                Assert.AreEqual(4, startOfScan.Components.Length); // RGGB
                ...
                Assert.AreEqual(0, startOfScan.Components[2].Dc);
                Assert.AreEqual(0, startOfScan.Components[2].Ac); // in lossless, this value is always zero

                Assert.AreEqual(4, startOfScan.Components[3].Id);
                Assert.AreEqual(1, startOfScan.Components[3].Dc);
                Assert.AreEqual(0, startOfScan.Components[3].Ac);
                ...
                Assert.AreEqual(15631360, cc);
                Assert.AreEqual(17966425, startOfImage.ImageData.DistFromEnd);
            }
        }
        ...
        private static void CreateBitmap(BinaryReader binaryReader, StartOfImage startOfImage, string outFile, uint offset, ushort[] slices)
        {
            ...
            Assert.AreEqual(2272u, height);                         // image height

            var samplesPerLine = startOfFrame.SamplesPerLine;
            Assert.AreEqual(3440u, startOfFrame.SamplesPerLine);    // image width

            var width = startOfFrame.Width;
            Assert.AreEqual(13760, startOfFrame.Width);
            Assert.AreEqual(samplesPerLine * 2, slices[0] * slices[1] + slices[2]);
            Assert.AreEqual(width, samplesPerLine * 4);
