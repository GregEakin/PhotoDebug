...
public class Canon7d2
{
    ...
    private static void DumpImage3Raw(string fileName)
    {
        ...
        Assert.Equal(new[] { (ushort)1, (ushort)2784, (ushort)2784 }, slices);

        binaryReader.BaseStream.Seek(offset, SeekOrigin.Begin);
        var startOfImage = new StartOfImage(binaryReader, offset, count);

        var startOfFrame = startOfImage.StartOfFrame;
        Assert.Equal(3708u, startOfFrame.ScanLines); // = 3840 + 110
        Assert.Equal(2784u, startOfFrame.SamplesPerLine); // = 5920 / 2
        Assert.Equal(5568, startOfFrame.Width); // = 5760 + 160

        Assert.Equal(14, startOfFrame.Precision); // RGGB

        // chrominance subsampling factors
        Assert.Equal(2, startOfFrame.Components.Length); // RGGB
        ...
        Assert.Equal(2, startOfFrame.Components.Sum(component => component.HFactor * component.VFactor));
        ...
        Assert.Equal(2, startOfScan.Components.Length); // RGGB
    ...
    private static void CreateBitmap(BinaryReader binaryReader, StartOfImage startOfImage, string outFile, uint offset, ushort[] slices)
    {
        ...
        Assert.Equal(3708u, height);                         // image height

        var samplesPerLine = startOfFrame.SamplesPerLine;
        Assert.Equal(2784u, startOfFrame.SamplesPerLine);    // image width

        var width = startOfFrame.Width;
        Assert.Equal(5568, startOfFrame.Width);
        Assert.Equal(samplesPerLine * 2, slices[0] * slices[1] + slices[2]);
        Assert.Equal(width, samplesPerLine * 2);
