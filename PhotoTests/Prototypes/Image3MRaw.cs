...
public class Image3MRaw
{
    ...
    private static void DumpImage3SRaw(string fileName)
    {
        ...
        Assert.Equal(2592u, startOfFrame.ScanLines);
        Assert.Equal(3888u, startOfFrame.SamplesPerLine);
        ...
        Assert.Equal(15, startOfFrame.Precision); // mraw/sraw1
        ...
        Assert.Equal(2, startOfFrame.Components[0].VFactor); // MRAW
    ...
    {
        Assert.Equal(samplesPerLine / 4, diff.Length);
    ...
    private static void MakeBitmap(DataBuf[][] memory, string folder, ushort[] slices)
    {
        ...
        Assert.Equal(2592, y);   // scan lines
        Assert.Equal(3888, x);   // samples per line

        Assert.Equal(3 * x, slices[0] * slices[1] + slices[2]);
        Assert.Equal(new[] { (ushort)8, (ushort)1296, (ushort)1296 }, slices);
        slices[1] /= 3;
        slices[2] /= 3;
