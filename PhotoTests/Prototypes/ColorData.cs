...
public class ColorData
{
    ...
    public void DumpColorData()
    {
        ...
        //Assert.Equal(1, tags.Entries.Length);
        //Assert.Equal(
        //    new ushort[] { 0x0000 }, tags.Entries.Select(e => e.TagId).ToArray());

        //var imageFileEntry = tags.Entries.Single(e => e.TagId == 0x0000 && e.TagType == ImageFileEntry.TagTypes.UByte);
        //Assert.Equal(4u, imageFileEntry.NumberOfValue);
        //// Assert.Equal(0x302u, imageFileEntry.ValuePointer);
        //var xmpData = RawImage.ReadBytes(binaryReader, imageFileEntry);
        //Assert.Equal(new byte[] { 0x01, 0x00, 0x0F, 0xA2 }, xmpData);
    }
}
