...
public class Image3SRawI
{
    ...
    public void DumpImage3SRawTest()
    {
        ...
        const string folder = @"\\Data\Photo\2016\2016-02-26\";
    ...
    private static void DumpImage3SRaw(string folder, string file)
    {
        ...
        Assert.Equal(1728u, startOfFrame.ScanLines); // height
        Assert.Equal(2592u, startOfFrame.SamplesPerLine); // width
        Assert.Equal(7776, startOfFrame.Width);
        Assert.Equal(startOfFrame.SamplesPerLine*3, startOfFrame.Width);
        Assert.Equal(startOfFrame.SamplesPerLine*2, slices[0]*slices[1] + slices[2]);
