...
public class Image3SRawII
{
    ...
    {
        const string fileName = @"\\Data\Photo\2016\2016-02-26\007.CR2";
    ...
    private static void DumpImage3SRaw(string fileName)
    {
        ...
        // Image #3 is a raw image compressed in ITU-T81 lossless JPEG
        {
            ...
            Assert.Equal(new ushort[] { 5, 864, 864 }, slices);
            ...
            Assert.Equal(2, startOfImage.HuffmanTable.Tables.Count);
            ...
            Assert.Equal(3, startOfFrame.Components.Length); // sraw/sraw2

            // J:a:b = 4:2:2, h/v = 2/1
            Assert.Equal(1, startOfFrame.Components[0].ComponentId);
            Assert.Equal(2, startOfFrame.Components[0].HFactor); // SRAW
    ...
    private static void MakeBitmap(DataBuf[][] memory, string folder, ushort[] slices)
    {
        ...
        Assert.Equal(1728, y);   // scan lines
        Assert.Equal(2592, x);   // samples per line

        Assert.Equal(2 * x, slices[0] * slices[1] + slices[2]);
