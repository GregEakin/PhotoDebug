...
public class Image3SRawIV
{
    ...
    private static void DumpImage3SRaw(string fileName)
    {
        ...
        Assert.Equal(1728u, startOfFrame.ScanLines);
        Assert.Equal(2592u, startOfFrame.SamplesPerLine);
        Assert.Equal(7776, startOfFrame.Width);

        Assert.Equal(15, startOfFrame.Precision); // sraw/sraw2

        // chrominance subsampling factors
        Assert.Equal(3, startOfFrame.Components.Length); // mraw/sraw1

        Assert.Equal(1, startOfFrame.Components[0].ComponentId);
        Assert.Equal(2, startOfFrame.Components[0].HFactor);
        Assert.Equal(1, startOfFrame.Components[0].VFactor); // SRAW
        ...
        Assert.Equal(3, startOfScan.Components.Length); // sraw/sraw2

        Assert.Equal(1, startOfScan.Components[0].Id);
        Assert.Equal(0, startOfScan.Components[0].Dc);
        Assert.Equal(0, startOfScan.Components[0].Ac);
        ...
        Assert.Equal(1, startOfScan.Components[2].Dc);
        Assert.Equal(0, startOfScan.Components[2].Ac);
    ...
    private static void CreateBitmap(BinaryReader binaryReader, StartOfImage startOfImage, string outFile, uint offset, ushort[] slices)
    {
        ...
        Assert.Equal(1728, height);              // image height

        var samplesPerLine = startOfFrame.SamplesPerLine;
        Assert.Equal(2592, samplesPerLine);      // image width
