    ...
    public class JpegImage
    {
        public static void TestImage()
        {
            ...
            using (var fileStream = File.Open(FileName, FileMode.Open, FileAccess.Read))
            {
                ...
                var address = directory.Entries.Single(e => e.TagId == 0x0111).ValuePointer;     // TIF_STRIP_OFFSETS For each strip, the byte offset of that strip.
                var length = directory.Entries.Single(e => e.TagId == 0x0117).ValuePointer;      // TIF_STRIP_BYTE_COUNTS For each strip, the number of bytes in the strip after compression.
